...
bugslist <- data.table::fread(params$bugslist) %>% as.data.frame()
...
plot_1 <- plot + scale_y_continuous(labels = scales::percent, expand = expansion(0)) +
  ...
  # facet_grid(~facet.label, scales="free_x", space="free_x",
  #           )

multi <- plot_grid(plot_1, GP_legend,  rel_widths = c(1, .25))
multi
ggsave("taxa-bar-phyum-genus-microshades.pdf",plot=multi,height=10,width=10, bg="transparent")
